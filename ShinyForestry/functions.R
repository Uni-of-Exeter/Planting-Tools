...
combine_foreach_rbind <- function(...) {
  mapply('rbind', ..., SIMPLIFY = FALSE)
                                    ...
                                    calories_livestock_arable<- function(shapefile, yield_data, matched_data_sf, FullTable) {
   
   # Load necessary libraries
   library(dplyr)
   library(sf)
   
   shapefile <- st_make_valid(shapefile)  # Ensure valid geometries
   
   ### ---- Step 1: Process Arable Crop Calories ---- ###
   ...
   # Define calorie conversion function for crops
   ...
   # Create a dataframe for arable calories
   df_calories <- data.frame(matrix(nrow = nrow(yield_data), ncol = ncol(yield_data)))
   colnames(df_calories) <- paste0("Arable_calories_", 2023:2050)
   
   # Compute calories for each crop
   for (i in 1:nrow(yield_data)) {
     for (j in 1:ncol(yield_data)) {
       crop <- rotations2050$rotation_1[j]  # Get crop type for the year
       df_calories[i, j] <- calculate_calories(yield_data[i, j], crop)  # Compute calories
     }
   }
   
   df_calories <- cbind(gid = d3$gid, df_calories)
   
   # Ensure 'gid' is character type for merging
   shapefile <- shapefile %>% mutate(gid = as.character(gid))
   df_calories <- df_calories %>% mutate(gid = as.character(gid))
   
   # Generate arable calorie column names
   calories_columns_arable <- paste0("Arable_calories_", 2023:2050)
   
   # Create an empty dataset with zeroed-out arable calorie columns
   full_arable_calories <- shapefile %>%
     select(gid, geometry) %>%
     bind_cols(as.data.frame(matrix(0, nrow = nrow(shapefile), ncol = length(calories_columns_arable))))
   
   # Assign column names
   colnames(full_arable_calories) <- c("gid", calories_columns_arable, "geometry")
   
   # Find indices where `gid` matches and replace values
   matched_indices_arable <- match(df_calories$gid, full_arable_calories$gid)
   full_arable_calories[matched_indices_arable, calories_columns_arable] <- df_calories[, calories_columns_arable]
   
   # Remove 'gid' and 'geometry' columns
   ...
   df_calories_livestock <- matched_data_sf %>% mutate(gid = as.character(gid))
 ...
 #yield_data = get(load("df_yield.RData")) # you need to load Yield Data
 #FullTable_m<- calories_livestock_arable(shapefile= sf::st_read("C:/Users/mh1176/University of Exeter/Mancini, Mattia - PCSE-WOFOST/ParcelData/land_parcels.shp"), # change your path
                                          #yield_data = yield_data,matched_data_sf=sf::st_read("df_livestock.geojson"), #you need load livestock data
