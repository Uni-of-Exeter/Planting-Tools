...
#* Runs in a future to avoid taking too much RAM
#* curl -X PUT -H "Accept: text/plain" localhost:<port>/initialization
#* @put /initialize
#* @serializer rds
#* @param LOG_LEVEL="info" # more --> less: debug / info / warning / error / none
...
function(res, LOG_LEVEL) {
  
  library(future)
  plan(futureplan, workers = 2)
  # plan(sequential)
  
  app_environment <- value(future({
  ...
  list2env(as.list(app_environment), envir = .GlobalEnv)
  
  res$status <- 200
  return(as.list(app_environment))
}
